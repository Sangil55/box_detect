...
def test_model(
    ...
    for B, batch in enumerate(val_loader):
        # print(B)
        inputs, labels = batch
        inputs = (inputs/255).to(torch.device('cuda:0'))
        inputs_2ch = cfg.ch_select2(inputs)
        labels = labels.to(torch.device('cuda:0'))


        val_dab.append(labels.cpu().detach())
        outputs,_ = model(inputs_2ch)
        abnormal_score = torch.norm(outputs-inputs_2ch)

        variance = torch.var(inputs_2ch)
        # print('varaince : ',variance)
        # print('ab score :', abnormal_score/variance)
        if abnormal_score/variance > threshold:
            predict = 1
        else:
            predict = 0
        ...
        y_list.append(labels[0].cpu().detach().int())
        pre_list.append(predict)
        

        # for idx,p_val in enumerate(predict):
        #     p = int(p_val.cpu().detach())
        dpath = 'd:\\src\\box_detect\\misprediction\\'+dataset_name+str(predict)
        img = inputs[0].cpu().detach()[0:3]
        img = np.swapaxes(np.swapaxes(img.numpy(),0,2),0,1)
        cv2.imwrite(os.path.join(dpath,str(counter)+'.jpeg'), img*255) 
        counter+=1
        #     counter += 1

    pp = np.array(pre_list)
    # pp = pre_list.cpu().detach().numpy()
...
if __name__ == '__main__':
    ...
    for i,target_datapath in enumerate(target_datapath_lst):
        ...
        iter_valid = torch.utils.data.DataLoader(dfvalset, batch_size=1, shuffle=False,
                                             pin_memory=True)
